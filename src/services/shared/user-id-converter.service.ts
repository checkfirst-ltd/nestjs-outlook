...
export class UserIdConverterService {
  ...
  /**
   * Check if a Microsoft user exists for the given external user ID
   *
   * @param externalUserId - The user ID from the host application
   * @returns True if an active Microsoft user exists
   */
  async existsByExternalId(externalUserId: string): Promise<boolean> {
    const user = await this.microsoftUserRepository.findOne({
      where: { externalUserId, isActive: true },
    });
  ...
  /**
   * Check if a Microsoft user exists for the given internal user ID
   *
   * @param internalUserId - The internal database ID (primary key)
   * @returns True if a Microsoft user exists
   */
  async existsByInternalId(internalUserId: number): Promise<boolean> {
    const user = await this.microsoftUserRepository.findOne({
      where: { id: internalUserId },
    });

...
export class MicrosoftAuthService {
  ...
  /**
   * Get Microsoft user token info by external user ID
   * 
   * this will access microsoft_users table and return the token info for the user
   * the token is stored when the user is authenticated and when the webhook subscriptions are created
   * 
   * @param externalUserId - External user ID from the host application
   * @returns Microsoft user token info
   */
  ...
  async getUserAccessTokenByExternalUserId(externalUserId: string): Promise<string> {
    try {
      ...
      return await this.processTokenInfo(userInfo, internalUser.id);
    } catch (error) {
   ...
   * @returns Valid access token string
   * @private This method is for internal use only. Use getUserAccessTokenByExternalUserId() instead.
   */
  private async getUserAccessTokenByInternalUserId(internalUserId: number | string): Promise<string> {
    try {
   ...
   *
   * **Most applications should use `getUserAccessTokenByExternalUserId()` instead**,
   ...
   *
   * @see getUserAccessTokenByExternalUserId For standard usage with automatic token refresh
   */

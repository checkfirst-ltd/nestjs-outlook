...
export class MicrosoftSubscriptionService {
  ...
  /**
   * Resolve a Graph access token for a user, matching their auth mode: the tenant's app-only
   * token when the user (or an explicit `tenantId`) is tenant-mapped, the user's delegated
   * token otherwise. This is the generic, auth-mode-aware entry point — prefer it over calling
   * `MicrosoftAuthService.getUserAccessToken` (delegated-only) directly.
   *
   ...
   */
  async resolveUserAccessToken(params: {
    internalUserId: number;
  ...
  async renewWebhookSubscription(
    ...
    try {
      ...
      const accessToken = await this.resolveUserAccessToken({ internalUserId });
      if (!accessToken) {
        throw new Error(
  ...
  async getActiveSubscriptionForUser(externalUserId: string): Promise<string | null> {
    try {
      ...
      // A CORRUPTED, SUBSCRIPTION_FAILED, or inactive user needs to re-authenticate —
      // report as not connected so the UI prompts a fresh OAuth flow.
      const user = await this.microsoftUserRepository.findOne({
        select: ['id', 'status', 'isActive'],
        where: { id: internalUserId },
      });
      if (!user || user.status !== MicrosoftUserStatus.ACTIVE || !user.isActive) {
        this.logger.debug(
          `[getActiveSubscriptionForUser] User ${externalUserId} is missing, not ACTIVE (status=${user?.status}), or inactive — reporting as not connected`
        );
        return null;
      }
  ...
  private async verifySubscriptionsForUser(
    ...
    try {
      ...
      const resolvedToken = await this.resolveUserAccessToken({ internalUserId: userId });
      if (!resolvedToken) {
